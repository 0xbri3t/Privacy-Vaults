...
export function DepositStage({ progress }: StageProps) {
  ...
  // Build full curve for coin path
  const fullCurve = useMemo(() => {
    const path1Start = new THREE.Vector3(USER_POS.x, USER_POS.y + 0.8, USER_POS.z)
    const path1End = POOL_POS.clone()
    const path2Start = POOL_POS.clone()
    const path2End = GEAR_POS.clone()
  ...
  const [coinPos, setCoinPos] = useState<[number, number, number]>([USER_POS.x, USER_POS.y + 0.8, USER_POS.z])
  ...
  useFrame(({ clock }) => {
    if (!groupRef.current) return
    groupRef.current.visible = progress < 0.27
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* ── Dashed Paths ── */}
      <DashedPath
        start={new THREE.Vector3(USER_POS.x, USER_POS.y + 0.8, USER_POS.z)}
        end={POOL_POS}
        arcHeight={0.8}
        dashCount={25}
        opacity={opacity}
